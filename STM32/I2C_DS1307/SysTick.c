...
void SysTick_Init(void){
  ...
  SysTick_CTRL_SR = 0x00000005;      // enable SysTick with core clock
}
// The delay parameter is in units of the 24 MHz core clock. (41.7 ns)
void SysTick_Wait(unsigned long delay){
  SysTick_RELOAD_VR = delay-1;  // number of counts to wait
  SysTick_CURRENT_VR = 0;       // any value written to CURRENT clears
  while((SysTick_CTRL_SR & 0x00010000)==0){ // wait for count flag
  }
}
// 10000us equals 10ms
