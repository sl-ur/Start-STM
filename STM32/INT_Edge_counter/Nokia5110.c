...
// Переделано под STM32F100BR, STM32VL Discovery
// Карпов Вячеслав
// 29 мая 2016 г.
...
#define DC_DATA_COMMAND       0x00000008  // Дата "1", Команда "0", РА3
#define RESET_HIGH_LOW        0x00000004  // Сброс "0", РА2
...
#define GPIOA_CRL			(*((volatile unsigned long *)0x40010800)) // Упраляющий регистр порта А, РА0 - РА7
#define GPIOA_CRH			(*((volatile unsigned long *)0x40010804)) // Упраляющий регистр порта А, РА8 - РА15
#define GPIOA_ODR			(*((volatile unsigned long *)0x4001080C)) // Регистр выходных данных порта А
#define GPIOA_BSRR    (*((volatile unsigned long *)0x40010810)) // Регистр установки/сброса бит порта А
...
void static lcdwrite(enum typeOfWrite type, unsigned char message){
    unsigned long delay;
    if(type == COMMAND){
                                   // wait until SPI1 not busy/transmit FIFO empty
    while((SPI1_SR & SPI1_SR_BSY) == SPI1_SR_BSY){};
    GPIOA_ODR &= ~DC_DATA_COMMAND; // Передача команды
    for(delay=0; delay<5; delay=delay+1); // Задержка между подачи Команды и запись информации мин 100 нс
    SPI1_DR = message;             // command out
                                   // wait until SPI1 not busy/transmit FIFO empty
    while((SPI1_SR & SPI1_SR_BSY) == SPI1_SR_BSY){};
  } else{
    while(((SPI1_SR & SPI1_SR_TXE) >> 1) == 0){}; // wait until transmit FIFO not full
    GPIOA_ODR |= DC_DATA_COMMAND;  // Передача информации
    for(delay=0; delay<5; delay=delay+1); // Задержка между подачи Команды и запись информации мин 100 нс
